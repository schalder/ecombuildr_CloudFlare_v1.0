...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // For all other requests (regular users), serve the static files
  try {
    // Try to serve static files
    const filePath = join(process.cwd(), 'dist', 'index.html');
    const html = readFileSync(filePath, 'utf8');
    
    // If it's a custom domain but not a social crawler, we can optionally inject SEO data
    if (isCustomDomain && !isSocialCrawler) {
      // For regular users on custom domains, we can inject SEO data client-side
      // This is handled by the existing client-side script in index.html
      console.log('Serving static HTML for regular user on custom domain');
    }
