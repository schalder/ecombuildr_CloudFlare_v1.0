...
export const CheckoutElementStyles: React.FC<CheckoutElementStylesProps> = ({ element, onStyleUpdate }) => {
  ...
  const { deviceType: tab, setDeviceType: setTab } = useDevicePreview();
  ...
  // Get effective values with proper inheritance (mobile inherits from tablet, tablet inherits from desktop)
  const getEffectiveValue = (responsiveStyles: any, property: string, fallback: string) => {
    if (tab === 'mobile') {
      return responsiveStyles.responsive?.mobile?.[property] || 
             responsiveStyles.responsive?.tablet?.[property] ||
             responsiveStyles.responsive?.desktop?.[property] || 
             fallback;
    }
    if (tab === 'tablet') {
      return responsiveStyles.responsive?.tablet?.[property] ||
             responsiveStyles.responsive?.desktop?.[property] || 
             fallback;
    }
  ...
  const headerStyles = ((element.styles as any)?.checkoutSectionHeader) || { responsive: { desktop: {}, tablet: {}, mobile: {} } } as any;
  ...
  const updateResponsive = (key: string, value: any) => {
    const next = {
      responsive: {
        ...
        tablet: { ...(styles.responsive?.tablet || {}) },
  ...
  const updateHeaderResponsive = (key: string, value: any) => {
    const next = {
      responsive: {
        ...
        tablet: { ...(headerStyles.responsive?.tablet || {}) },
    ...
    <div className="space-y-4">
      ...
      <CollapsibleGroup title="Spacing" isOpen={spacingOpen} onToggle={setSpacingOpen}>
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as any)}>
          ...
          <TabsContent value="tablet" className="space-y-4 mt-3">
            <div className="space-y-3">
              <h5 className="text-xs font-medium">Margin</h5>
              <SpacingControl
                element={element}
