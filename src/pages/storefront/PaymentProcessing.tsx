...
export const PaymentProcessing: React.FC = () => {
  ...
  const handleDeferredOrderCreation = async () => {
    ...
    // ✅ For Stripe payments, handle immediately (order already exists, no need for store/checkout data)
    // This must happen BEFORE the store loading check, since Stripe orders are created before payment
    if (paymentMethod === 'stripe' && tempId) {
      try {
        ...
        if (fetchError || !orderData?.order) {
          ...
          // ✅ CRITICAL FALLBACK: Even if fetch fails, redirect using tempId as order ID
          // The order exists (we created it before payment), so we can still redirect
          console.log('PaymentProcessing: Stripe order fetch failed, but redirecting anyway with tempId as order ID');
          const fallbackToken = crypto.randomUUID().replace(/-/g, '');
          sessionStorage.removeItem('pending_checkout');
          clearCart();
          window.location.href = paths.orderConfirmation(tempId, fallbackToken);
        ...
        // ✅ CRITICAL FALLBACK: Even on error, redirect using tempId as order ID
        // The order exists (we created it before payment), so we can still redirect
        console.log('PaymentProcessing: Stripe order processing error, but redirecting anyway with tempId as order ID');
        const fallbackToken = crypto.randomUUID().replace(/-/g, '');
        sessionStorage.removeItem('pending_checkout');
        clearCart();
        window.location.href = paths.orderConfirmation(tempId, fallbackToken);
    ...
    try {
      ...
      if (error) {
        ...
        // ✅ FIX: Only redirect on actual failure, with fallback to tempId
    ...
    try {
      ...
      if (orderToken) {
        try {
          ...
          if (error) {
            console.error('PaymentProcessing: get-order-public error:', error);
            // ✅ FALLBACK: If get-order-public fails, redirect to order confirmation anyway
            // The order exists, so we can redirect with a generated token
            console.log('PaymentProcessing: get-order-public failed, redirecting with fallback token');
            const fallbackToken = crypto.randomUUID().replace(/-/g, '');
            sessionStorage.removeItem('pending_checkout');
            clearCart();
            window.location.replace(paths.orderConfirmation(orderId, fallbackToken));
            return;
          }
          ...
          console.error('PaymentProcessing: Error fetching order:', err);
          // ✅ FALLBACK: Redirect even on error - order exists, so we can redirect
          const fallbackToken = crypto.randomUUID().replace(/-/g, '');
          sessionStorage.removeItem('pending_checkout');
          clearCart();
          window.location.replace(paths.orderConfirmation(orderId, fallbackToken));
