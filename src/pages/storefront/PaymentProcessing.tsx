...
export const PaymentProcessing: React.FC<{ funnelContext?: any }> = ({ funnelContext }) => {
  ...
  const handleDeferredOrderCreation = async () => {
    ...
    try {
      ...
      if (data?.success && data?.order) {
        ...
        if (orderFunnelId || funnelContext) {
          console.log('PaymentProcessing: Order from funnel detected', { 
            funnelId: orderFunnelId || funnelContext?.funnelId, 
            orderId: data.order.id,
            hasFunnelContext: !!funnelContext
          });
          
          try {
            let nextStepSlug = null;
            let funnelId = orderFunnelId || funnelContext?.funnelId;
            
            // Use funnel context if available (from custom domain wrapper)
            if (funnelContext?.nextStepSlug) {
              nextStepSlug = funnelContext.nextStepSlug;
              console.log('PaymentProcessing: Using funnel context for redirect', { nextStepSlug, funnelId });
            } else {
              // Fallback to database lookup (for app environment)
            ...
            if (nextStepSlug) {
              ...
              if (isAppEnvironment) {
                ...
                const nextUrl = `/funnel/${funnelId}/${nextStepSlug}?orderId=${data.order.id}&ot=${newOrderToken}`;
                console.log(`PaymentProcessing: Redirecting to next funnel step (app): ${nextUrl}`);
                window.location.href = nextUrl;
                return;
              } else {
                // Custom domain: use clean paths
                const nextUrl = `/${nextStepSlug}?orderId=${data.order.id}&ot=${newOrderToken}`;
  ...
  const verifyPayment = async () => {
    ...
    try {
      ...
      if (data.paymentStatus === 'success') {
        ...
        // Check if this order came from a funnel and handle funnel-specific redirect
        if (order.funnel_id || funnelContext) {
          console.log('PaymentProcessing: Payment verification - Order from funnel detected', { 
            funnelId: order.funnel_id || funnelContext?.funnelId, 
            orderId: order.id,
            hasFunnelContext: !!funnelContext
          });
          ...
          try {
            let nextStepSlug = null;
            let funnelId = order.funnel_id || funnelContext?.funnelId;
            
            // Use funnel context if available (from custom domain wrapper)
            if (funnelContext?.nextStepSlug) {
              nextStepSlug = funnelContext.nextStepSlug;
              console.log('PaymentProcessing: Payment verification - Using funnel context for redirect', { nextStepSlug, funnelId });
            } else {
              // Fallback to database lookup (for app environment)
              const { data: currentStep, error: stepError } = await supabase
                .from('funnel_steps')
            ...
            if (nextStepSlug) {
              ...
              if (isAppEnvironment) {
                // App/sandbox: use funnel-aware paths
                const nextUrl = `/funnel/${funnelId}/${nextStepSlug}?orderId=${order.id}&ot=${orderToken}`;
                console.log(`PaymentProcessing: Payment verification - Redirecting to next funnel step (app): ${nextUrl}`);
                window.location.href = nextUrl;
                return;
              } else {
                // Custom domain: use clean paths
                const nextUrl = `/${nextStepSlug}?orderId=${order.id}&ot=${orderToken}`;
