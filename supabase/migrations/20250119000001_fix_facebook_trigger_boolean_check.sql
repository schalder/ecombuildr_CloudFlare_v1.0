-- Fix Facebook trigger boolean check
-- The trigger was checking for string 'true' but JSONB stores boolean true
-- This caused the trigger to exit early and never call the edge function
...
BEGIN
  -- ✅ FIX: Check if Facebook is configured (handle both boolean and string)
  -- Extract the configured value and check if it's truthy
  facebook_configured := COALESCE(
    (NEW.event_data->'_providers'->'facebook'->>'configured')::boolean,
    CASE 
      WHEN NEW.event_data->'_providers'->'facebook'->>'configured' IN ('true', 'True', 'TRUE', '1') THEN true
      ELSE false
    END,
    false
  );
  
  IF NOT facebook_configured THEN
    RETURN NEW;
  END IF;
