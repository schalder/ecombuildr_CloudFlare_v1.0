-- Fix Facebook trigger to check server-side configuration directly
-- Don't rely on _providers.facebook.configured (client-side flag)
-- This ensures server-side events fire even if browser pixel isn't configured
...
BEGIN
  -- ✅ CRITICAL FIX: Check server-side configuration directly from database
  -- Don't check _providers.facebook.configured (that's client-side only)
  ...
  -- ✅ CRITICAL: Only skip if server-side tracking is NOT enabled
  -- Don't check _providers.facebook.configured (client-side flag)
