...
-- Function to check limits before product insertion
...
-- Function to update count after product deletion
CREATE OR REPLACE FUNCTION public.decrease_product_count()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
-- Function to check limits before page insertion
CREATE OR REPLACE FUNCTION public.check_page_limits()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
-- Function to update count after page deletion
CREATE OR REPLACE FUNCTION public.decrease_page_count()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
-- Function to check website/funnel/order limits
...
DROP TRIGGER IF EXISTS trigger_check_product_limits ON public.products;
CREATE TRIGGER trigger_check_product_limits
  BEFORE INSERT ON public.products
  FOR EACH ROW EXECUTE FUNCTION public.check_product_limits();

DROP TRIGGER IF EXISTS trigger_decrease_product_count ON public.products;
CREATE TRIGGER trigger_decrease_product_count
  AFTER DELETE ON public.products
  FOR EACH ROW EXECUTE FUNCTION public.decrease_product_count();

-- Create triggers for page count tracking
DROP TRIGGER IF EXISTS trigger_check_website_page_limits ON public.website_pages;
CREATE TRIGGER trigger_check_website_page_limits
  BEFORE INSERT ON public.website_pages
  FOR EACH ROW EXECUTE FUNCTION public.check_page_limits();

DROP TRIGGER IF EXISTS trigger_decrease_website_page_count ON public.website_pages;
CREATE TRIGGER trigger_decrease_website_page_count
  AFTER DELETE ON public.website_pages
  FOR EACH ROW EXECUTE FUNCTION public.decrease_page_count();

DROP TRIGGER IF EXISTS trigger_check_funnel_step_limits ON public.funnel_steps;
CREATE TRIGGER trigger_check_funnel_step_limits
  BEFORE INSERT ON public.funnel_steps
  FOR EACH ROW EXECUTE FUNCTION public.check_page_limits();

DROP TRIGGER IF EXISTS trigger_decrease_funnel_step_count ON public.funnel_steps;
CREATE TRIGGER trigger_decrease_funnel_step_count
